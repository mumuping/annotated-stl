...
// 拼接，before_first 表示目标迭代器开始的前一个
// before_last 表示目标迭代器结束的前一个
// 拼接的新位置位于 pos 后
...
{
  _Slist_node_base* __before_last = __slist_previous(__head, 0);// 找到尾节点
...
struct _Slist_iterator_base
{
  ...
  void _M_incr() { _M_node = _M_node->_M_next; }	// 指向下一个 node
...
protected:
  typename _Alloc_traits<_Slist_node<_Tp>,_Allocator>::allocator_type
           _M_node_allocator;
  _Slist_node_base _M_head;// 这里只存有一个指针，没有元素，_Slist_node 中才有真正的元素
  // 因此析构时不需要单独的进行释放
};
                             ...
                             _Alloc_traits<_Tp, _Alloc>::_S_instanceless>
{
  ...
  _Slist_base(const allocator_type& __a)
	...
	// 析构函数
  ~_Slist_base() { _M_erase_after(&this->_M_head, 0); } // destroy 所有元素，并回收空间
...
// 拷贝构造
  ...
  slist& operator= (const slist& __x); // 操作符重载
