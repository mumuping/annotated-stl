...
// 拷贝
// 如果是 input_iterator 则使用的是 first!=last 进行比较，
// 如果是 random_iterator 则使用的是 distance: n>0 判断，
// 显然 random_iterator 的速度更快
...
// 对于 trivial 类型，将使用 memmove（该函数允许目的和源地址重叠）
template <class _Tp>
inline _Tp*
...
// 如果是 unsigned char 或 signed char 或 char 类型，则调用 memset
...
// [first1, last1) 和 [first2, last2) 第一个不匹配的地方
...
// 特化版本，如果是 unsigned char 类型，则通过 memcmp 比较
inline bool 
...
// 如果是 unsigned char 类型，则调用 memcmp 函数
inline int
